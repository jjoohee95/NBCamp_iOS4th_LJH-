...
while true {
    ...
    case 2:
        ...
        guard let num1 = Double(readLine() ?? "0") else {
            print("Error: 숫자를 입력하세요.")
            continue // 루프의 처음으로 돌아가서 다시 선택을 받음
        }
        print("두 번째 숫자를 입력하세요 : ", terminator: "")
        guard let num2 = Double(readLine() ?? "0") else {
            print("Error: 숫자를 입력하세요.")
            continue
        }
        print("연산자를 선택하세요 (+, -, *, /) : ", terminator: "")
        guard let oper = readLine(), let operation = oper.first else {
